...
describe Grid do
  ...
  describe '#add_livecell' do
    ...
    it 'can add cell to grid' do
      ...
      expect { @grid.add_livecell(nil, nil, livecell) }.to change { @grid.livecells_array.length }.by(1)
    end

    it 'will not add live cell to list if one exists at coordinate' do
      allow(livecell).to receive(:coordinate) { [0, 0] }
      @grid.add_livecell(nil, nil, livecell)
      expect { @grid.add_livecell(nil, nil, livecell) }.to change { @grid.livecells_array.length }.by(0)
    end
  end
  ...
  describe '#livecellneighbournumberhash' do
    ...
    it 'creates hash for single live cell' do
      ...
      expect(@grid.livecell_neighbour_number_hash).to include([0, 0] => [0, true],
                                                              [0, 1] => [1, false],
                                                              [0, -1] => [1, false],
                                                              [-1, -1] => [1, false],
                                                              [-1, 0] => [1, false],
    ...
    it 'creates hash for multiple live cells' do
      ...
      expect(@grid.livecell_neighbour_number_hash).to include([-1, -1] => [1, false],
                                                              [-1, 0] => [1, false],
                                                              [-1, 1] => [1, false],
                                                              [-1, 2] => [0, false],
                                                              [0, -1] => [1, false],
